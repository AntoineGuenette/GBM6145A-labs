function update_meanAct_results(input_path, output_path)
    ...
    % Create a structure for temporary storage
    storage = struct();
    ...
    for i = 2:size(C1, 1)
        ...
        % Replace dash by underscore to create a valid name
        safe_key = replace(safe_key, "-", "_");
    
        % Initialize the key in the storage if it does not exist
    ...
    % Compute means and fill output cell array
    ...
    for f = 1:length(fields)
        ...
        if ~isempty(data.Baseline) && ~isempty(data.Device)
            ...
            diff_perc = compute_EMG_diff(avg_J, avg_D);
